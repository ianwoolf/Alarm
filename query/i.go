...
// 	"github.com/lodastack/alarm/influx"
// )
...
// 	rawQuery := fmt.Sprintf("SELECT %s(\"value\") FROM \"%s\" WHERE time > %sms and time < %sms  fill(%s)",
// 		fn, measurement, start, end, fill)
// 	if where != "" {
// 		rawQuery = fmt.Sprintf("SELECT %s(\"value\") FROM \"%s\" WHERE %s AND time > %sms and time < %sms  %s fill(%s)",
// 			fn, measurement, where, start, end, strings.Join(filterTags, ","), fill)
// 	}
// 	return rawQuery, nil
// }
